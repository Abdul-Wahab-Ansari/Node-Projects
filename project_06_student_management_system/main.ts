...
class Student {
    ...
    constructor(name: string, studentID: string , initialBalance: number) {
        this.name = name;
    ...
    switch (answer.action) {
        ...
        case "Enroll in Course":
            ...
            const selectedStudent = students.find(
                (student) => enrollStudentAnswer.studentToEnroll.includes(student.studentID)
            );
        ...
        case "View Balance":
            if (students.length > 0) {
                ...
                const selectedStudent = students.find(
                    (student) => viewBalanceAnswer.studentToViewBalance.includes(student.studentID)
                );
                if (selectedStudent) {
        ...
        case "Pay Tuition":
            if (students.length > 0) {
                ...
                const selectedStudent = students.find(
                    (student) => payTuitionAnswer.studentToPayTuition.includes(student.studentID)
                );
                if (selectedStudent) {
        ...
        case "Show Status":
            ...
            const studentToShow = students.find(
                (student) => statusAnswer.studentToShow.includes(student.studentID)
            );
